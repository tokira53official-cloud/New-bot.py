...
# If bot.py is running, AI is available.
# If bot.py is stopped, AI is naturally offline.
...
PRIMECLOUD_AI_SYSTEM_PROMPT = """
You are PrimeCloud AI, the official AI assistant of PrimeCloud Cloud Services.

IDENTITY:
Your name is PrimeCloud AI.
You are the AI assistant of PrimeCloud.
...
and suggest PrimeCloud paid plans or available invite/referral plans.
...
# Money is stored as INR balance. Coupon claims and VPS plan claims are
# persisted separately so the new billing system does not disturb old VPS data.
...
def load_billing_data():
    try:
        with open("billing_data.json", "r") as f:
            ...
            data.setdefault("plan_claims", {})
            data.setdefault("earning_tasks", {})
            data.setdefault("afk", {})
            return data
    except (FileNotFoundError, json.JSONDecodeError, ValueError):
        return {"coupons": {}, "plan_claims": {}, "earning_tasks": {}, "afk": {}}

billing_data = load_billing_data()

VPS_PLANS = {
    "2gb":  {"ram": 2,  "cpu": 1, "disk": 20,  "price": 150},
    "4gb":  {"ram": 4,  "cpu": 1, "disk": 30,  "price": 300},
    "6gb":  {"ram": 6,  "cpu": 1, "disk": 30,  "price": 500},
    "8gb":  {"ram": 8,  "cpu": 1, "disk": 40,  "price": 750},
    "10gb": {"ram": 10, "cpu": 2, "disk": 50,  "price": 1000},
    "12gb": {"ram": 12, "cpu": 2, "disk": 60,  "price": 1500},
    "16gb": {"ram": 16, "cpu": 2, "disk": 70,  "price": 2500},
    "24gb": {"ram": 24, "cpu": 4, "disk": 100, "price": 5000},
    "32gb": {"ram": 32, "cpu": 4, "disk": 120, "price": 10000},
}

PLAN_CLAIM_LIMIT = 2
MONTH_DAYS = 30
AFK_RATE_PER_MINUTE = 20
TASK_MIN_MESSAGES = 50
TASK_MAX_MESSAGES = 100
TASK_MIN_REWARD = 200
TASK_MAX_REWARD = 1000

def save_billing_data():
    try:
        with open("billing_data.json", "w") as f:
            json.dump(billing_data, f, indent=4)
    except Exception as e:
        logger.error(f"Error saving billing data: {e}")

def get_balance(user_id):
    return float(user_data.get(str(user_id), {}).get("balance", 0))

def set_balance(user_id, amount):
    user_id = str(user_id)
    user_data.setdefault(user_id, {})
    user_data[user_id]["balance"] = round(float(amount), 2)

def parse_expiry(value):
    """Parse coupon expiry such as 30d, 12h, 45m, or an ISO datetime."""
    value = str(value).strip().lower()
    m = re.fullmatch(r"(\d+(?:\.\d+)?)(m|h|d|w)", value)
    if m:
...
def plan_key(plan):
    return str(plan).lower().replace(" ", "").replace("gb", "gb")
...
# ─── PrimeCloud AI Request ────────────────────────────────────────────────────

async def primecloud_ai_answer(question):
    ...
    for model in models:
        payload = {
            ...
            'messages': [
                {
                    'role': 'system',
                    'content': PRIMECLOUD_AI_SYSTEM_PROMPT
                },
                {
                    'role': 'user',
                    'content': question
        ...
        request = urllib.request.Request(
            ...
            headers={
                ...
                'HTTP-Referer': 'https://primecloud.host',
                'X-Title': 'PrimeCloud AI'
            },
            method='POST'
        )
        ...
        try:
            ...
            logger.info(f'PrimeCloud AI answered using model: {model}')
...
def primecloud_ai_direct_answer(question):
    ...
    if any(x in q for x in creator_words):
        return "Yes I made by NotRyxenYT."

    if any(x in q for x in vps_words):
        return (
            "Sorry, main directly VPS nahi de sakta. 😅\n\n"
            "Aap PrimeCloud ke **paid plans** ya available "
...
async def manage_vps(ctx, user: discord.Member = None):
    ...
    if user:
        ...
        vps_list = vps_data.get(user_id, [])
        if not vps_list:
            embed = create_embed("No VPS Found", "You don't have any VPS. Use `!deploy` to purchase one.", 0xff3366)
...
async def afk_command(ctx):
    """Toggle AFK earning mode. Earns INR while the user remains AFK."""
    if not ctx.guild:
        return
    uid = str(ctx.author.id)
    afk = billing_data.setdefault("afk", {})
    now = datetime.utcnow().timestamp()

    current = afk.get(uid)
    if current and current.get("started_at"):
        elapsed = max(0, int(now - float(current["started_at"])))
        minutes = elapsed // 60
        if minutes > 0:
            reward = minutes * AFK_RATE_PER_MINUTE
            set_balance(uid, get_balance(uid) + reward)
            current["started_at"] = now
            save_data()
            await ctx.send(embed=create_success_embed(
                "💤 AFK Reward Claimed",
                f"{ctx.author.mention} earned **₹{reward:,}** for **{minutes} minute(s)** of AFK.\n"
                f"Balance: **₹{get_balance(uid):,.2f}**"
            ))
            return
        await ctx.send(embed=create_info_embed("💤 Already AFK", "Stay AFK longer to earn balance."))
        return

    afk[uid] = {"started_at": now}
    save_data()
    await ctx.send(embed=create_success_embed(
        "💤 AFK Mode Started",
        f"{ctx.author.mention} is now marked as **AFK**.\n"
        f"Earn rate: **₹{AFK_RATE_PER_MINUTE}/minute**.\n"
        "Run `!afk` again later to collect your earned balance."
    ))


@bot.command(name='task')
async def task_command(ctx):
    """Start a random message/code earning task."""
    uid = str(ctx.author.id)
    active = billing_data.setdefault("earning_tasks", {})
    if uid in active and not active[uid].get("completed"):
        task = active[uid]
        await ctx.send(embed=create_info_embed(
            "🎯 Task Already Active",
            f"Send the code `{task['code']}` in chat **{task['target']} times**.\n"
            f"Progress: **{task['count']}/{task['target']}**"
        ))
        return

    target = random.randint(TASK_MIN_MESSAGES, TASK_MAX_MESSAGES)
    ...
    await ctx.send(embed=create_success_embed(
        "🎯 New Earning Task",
        f"Send this code as a message in **any server channel**:\n\n`{code}`\n\n"
        f"Required messages: **{target}**\n"
        f"Reward: **₹{TASK_MIN_REWARD}–₹{TASK_MAX_REWARD}** (random luck)\n\n"
        "The bot will track your progress automatically."
    ))


@bot.command(name='baladd')
@is_admin()
async def baladd(ctx, member: discord.Member, amount: float):
    """Admin-only command to add INR balance."""
    if amount <= 0:
        await ctx.send(embed=create_error_embed("Invalid Amount", "Amount must be greater than ₹0."))
        return
    uid = str(member.id)
    new_balance = get_balance(uid) + amount
    set_balance(uid, new_balance)
    save_data()
    await ctx.send(embed=create_success_embed(
        "💰 Balance Added",
        f"Added **₹{amount:,.2f}** to {member.mention}.\n"
...
async def show_plans(ctx):
    """Show PrimeCloud monthly VPS plans."""
    embed = create_embed(
        "💳 PrimeCloud Billing • VPS Plans",
        "All plans are billed **per month**. Each user can deploy each plan up to **2 times**.",
        0x00ccff
    )
    for key, plan in VPS_PLANS.items():
        embed.add_field(
            name=f"🖥️ {plan['ram']}GB RAM",
            value=(
                f"**₹{plan['price']}/month**\n"
                f"CPU: **{plan['cpu']} core{'s' if plan['cpu'] != 1 else ''}**\n"
                f"Disk: **{plan['disk']}GB**\n"
                f"Claims: **{PLAN_CLAIM_LIMIT}/user**"
            ),
            inline=True
        )
    embed.add_field(
        name="⚡ Deploy",
        value="Use `!deploy <plan>` — Example: `!deploy 4gb`",
        inline=False
    )
    await ctx.send(embed=embed)
...
async def balance(ctx):
    """Show the user's INR billing balance."""
    amount = get_balance(ctx.author.id)
    await ctx.send(embed=create_info_embed(
        "💰 Billing Balance",
        f"{ctx.author.mention}, your balance is **₹{amount:,.2f}**."
    ))


@bot.command(name='baltop')
async def balance_top(ctx):
    """Show the users with the highest INR balance."""
    rows = sorted(
        ((uid, get_balance(uid)) for uid in user_data),
        key=lambda x: x[1],
        reverse=True
    )
    rows = [(uid, amount) for uid, amount in rows if amount > 0][:10]
    lines = []
    medals = ["🥇", "🥈", "🥉"] + ["🏅"] * 7
    for i, (uid, amount) in enumerate(rows):
        try:
            member = await bot.fetch_user(int(uid))
            name = member.display_name
        except Exception:
            name = f"User#{str(uid)[:6]}"
        lines.append(f"{medals[i]} **{name}** — ₹{amount:,.2f}")
    await ctx.send(embed=create_embed(
        "🏆 Balance Top",
        "\n".join(lines) if lines else "No users have a positive balance yet.",
        0xffd700
    ))


@bot.group(name='coupon', invoke_without_command=True)
@is_admin()
async def coupon(ctx):
    """Admin coupon management."""
    await ctx.send(embed=create_info_embed(
        "🎟️ Coupon",
        "Use `!coupon create <code> <expiry> <slots> [amount]` or `!coupon delete <id>`."
    ))


@coupon.command(name='create')
@is_admin()
async def coupon_create(ctx, code: str, expiry: str, slots: int, amount: Optional[float] = None):
    """Create a billing balance coupon."""
    if slots <= 0:
        await ctx.send(embed=create_error_embed("Invalid Slots", "Slots must be greater than 0."))
        return

    expiry_ts = parse_expiry(expiry)
    if expiry_ts is None or expiry_ts <= datetime.utcnow().timestamp():
        await ctx.send(embed=create_error_embed(
            "Invalid Expiry",
            "Use formats like `30d`, `12h`, `60m`, `2w`, or an ISO datetime."
        ))
        return
    ...
    if not re.fullmatch(r"[A-Z0-9_-]{2,64}", code):
        await ctx.send(embed=create_error_embed(
            "Invalid Code",
            "Coupon code may contain A-Z, 0-9, `_` and `-`."
        ))
        return

    if code in billing_data["coupons"]:
        await ctx.send(embed=create_error_embed("Already Exists", f"Coupon `{code}` already exists."))
        return

    # Amount is needed because the requested create syntax has no value field.
    # If omitted, ask the creator once so the command remains easy to use.
    if amount is None:
        await ctx.send("💰 Send the coupon amount in INR (example: `100`) within 30 seconds.")
        try:
            ...
            await ctx.send(embed=create_error_embed("Cancelled", "No valid amount was provided."))
            return
    ...
    coupon_id = str(random.randint(100000, 999999))
    while coupon_id in billing_data["coupons"]:
        coupon_id = str(random.randint(100000, 999999))

    billing_data["coupons"][code] = {
        "id": coupon_id,
    ...
    await ctx.send(embed=create_success_embed(
        "🎟️ Coupon Created",
        f"Code: `{code}`\nID: `{coupon_id}`\nAmount: **₹{amount:,.2f}**\n"
        f"Slots: **{slots}**\nExpires: <t:{int(expiry_ts)}:R>"
    ))


@coupon.command(name='delete')
@is_admin()
async def coupon_delete(ctx, coupon_id: str):
    """Delete a coupon by its ID."""
    found = None
    for code, data in billing_data["coupons"].items():
        if str(data.get("id")) == str(coupon_id):
            found = code
            break
    if not found:
        await ctx.send(embed=create_error_embed("Not Found", f"No coupon found with ID `{coupon_id}`."))
        return
    del billing_data["coupons"][found]
    save_data()
    await ctx.send(embed=create_success_embed("🗑️ Coupon Deleted", f"Coupon `{found}` has been deleted."))


@bot.command(name='redeem')
async def redeem_coupon(ctx, code: str):
    """Redeem a billing coupon once per user."""
    code = code.strip().upper()
    coupon = billing_data["coupons"].get(code)
    if not coupon:
        await ctx.send(embed=create_error_embed("Invalid Coupon", "That coupon does not exist."))
        return

    now = datetime.utcnow().timestamp()
    if now >= float(coupon["expires_at"]):
        await ctx.send(embed=create_error_embed("Coupon Expired", f"Coupon `{code}` has expired."))
        return

    if coupon["claimed"] >= coupon["slots"]:
        await ctx.send(embed=create_error_embed("Coupon Full", f"Coupon `{code}` has no slots left."))
        return

    uid = str(ctx.author.id)
    if uid in coupon.get("claimed_users", []):
        await ctx.send(embed=create_error_embed("Already Redeemed", "You have already redeemed this coupon."))
        return

    amount = float(coupon["amount"])
    set_balance(uid, get_balance(uid) + amount)
    coupon.setdefault("claimed_users", []).append(uid)
    coupon["claimed"] += 1
    save_data()

    await ctx.send(embed=create_success_embed(
        "🎉 Coupon Redeemed",
        f"{ctx.author.mention} received **₹{amount:,.2f}**.\n"
        f"New balance: **₹{get_balance(uid):,.2f}**\n"
        f"Remaining slots: **{coupon['slots'] - coupon['claimed']}**"
    ))


@bot.command(name='deploy')
async def deploy_plan(ctx, plan: str):
    """Deploy a monthly VPS plan using billing balance."""
    key = plan_key(plan)
    if key not in VPS_PLANS:
        await ctx.send(embed=create_error_embed(
            "Invalid Plan",
            "Use `!plans` to see the available plans."
        ))
        return

    uid = str(ctx.author.id)
    plan_info = VPS_PLANS[key]
    claims = billing_data["plan_claims"].setdefault(uid, {})
    used = int(claims.get(key, 0))
    if used >= PLAN_CLAIM_LIMIT:
        await ctx.send(embed=create_error_embed(
            "Claim Limit Reached",
            f"You have already claimed the **{plan_info['ram']}GB** plan {PLAN_CLAIM_LIMIT} times."
        ))
        return
    ...
    if balance < plan_info["price"]:
        await ctx.send(embed=create_error_embed(
            ...
            f"Required: **₹{plan_info['price']}**\n"
            f"Your balance: **₹{balance:,.2f}**\n"
            f"Missing: **₹{plan_info['price'] - balance:,.2f}**"
        ))
        return

    # Keep the same Docker VPS backend already used by the bot.
    container_name = f"primecloud-{uid[-6:]}-{random.randint(1000, 9999)}"
    password = generate_password()
    try:
        await ctx.send(embed=create_info_embed(
            "🚀 Deploying VPS",
            f"Plan: **{plan_info['ram']}GB RAM**\n"
            f"CPU: **{plan_info['cpu']} core{'s' if plan_info['cpu'] != 1 else ''}**\n"
            f"Disk: **{plan_info['disk']}GB**\n"
            "Please wait while Docker provisions your VPS..."
        ))
        await create_docker_container(
            container_name,
            plan_info["ram"] * 1024,
            ...
            password,
            disk_gb=plan_info["disk"],
            image=DOCKER_IMAGE
        )
    except Exception as e:
        logger.error(f"Plan deployment failed for {ctx.author.id}: {e}")
        await ctx.send(embed=create_error_embed(
            "Deployment Failed",
            "The VPS could not be created, so **no balance was deducted**."
        ))
        return

    expires = datetime.utcnow().timestamp() + MONTH_DAYS * 86400
    vps = {
        "container_name": container_name,
        "ram": f"{plan_info['ram']}GB",
        "cpu": str(plan_info["cpu"]),
        "storage": f"{plan_info['disk']}GB",
        "status": "running",
        "ssh_password": password,
        "ssh_port": ssh_port,
        "os": DOCKER_IMAGE,
        ...
        "expires": datetime.utcfromtimestamp(expires).isoformat(),
        "billing_type": "monthly"
    }
    vps_data.setdefault(uid, []).append(vps)

    set_balance(uid, balance - plan_info["price"])
    claims[key] = used + 1
    save_data()

    await ctx.send(embed=create_success_embed(
        "✅ VPS Deployed",
        f"Your **{plan_info['ram']}GB** VPS is now running.",
        )
    )
    embed = create_info_embed("🖥️ VPS Details", "")
    embed.add_field(name="Container", value=f"`{container_name}`", inline=False)
    embed.add_field(name="Resources", value=f"{plan_info['ram']}GB RAM • {plan_info['cpu']} Core • {plan_info['disk']}GB Disk", inline=False)
    embed.add_field(name="Monthly Billing", value=f"**₹{plan_info['price']}** deducted • Next expiry <t:{int(expires)}:R>", inline=False)
    embed.add_field(name="SSH Password", value=f"`{password}`", inline=False)
    await ctx.send(embed=embed)
...
async def ports_help(ctx):
    ...
    embed = create_embed(
        "📚 PrimeCloud Help - 🔌 Port Forwarding",
    ...
    embed.set_footer(text="PrimeCloud • Cloud Services")
...
def build_help_pages(is_user_admin, is_user_main_admin):
    ...
    user_embed = create_embed("👤 User Commands", "PrimeCloud user commands:", 0x00ff88)
    user_embed.add_field(name="🖥️ VPS Management", value=(
        "`!plans` — View monthly VPS plans\n"
        "`!deploy <plan>` — Deploy a VPS\n"
        "`!manage` — Manage your VPS\n"
        "`!myinfo` — View your profile & VPS summary"
    ), inline=False)
    user_embed.add_field(name="💳 Billing & Coupon", value=(
        "`!bal` — Check your money balance\n"
        "`!baltop` — Show users with the highest balance\n"
        "`!redeem <code>` — Redeem a coupon\n"
        "`!plans` — View VPS prices and resources"
    ), inline=False)
    user_embed.add_field(name="🔧 VPS Tools", value=(
        "`!rename-vps <#> <new_name>` — Rename a VPS\n"
        "`!vps-note <#> <note>` — Add a VPS note\n"
        "`!ping-vps <#>` — Ping a VPS\n"
        "`!uptime-vps <#>` — Check VPS uptime\n"
        "`!ports` — Port forwarding tools"
    ), inline=False)
    pages["user"] = user_embed

    billing_embed = create_embed(
        "💳 Billing & Coupon",
        "PrimeCloud monthly VPS billing and coupon commands.",
        0xffaa00
    )
    billing_embed.add_field(name="💰 Balance", value=(
        "`!bal` — Check your INR balance\n"
        "`!baltop` — Top users by balance"
    ), inline=False)
    billing_embed.add_field(name="🎟️ Coupon", value=(
        "`!redeem <code>` — Redeem a coupon once\n"
        "`!coupon create <code> <expiry> <slots> [amount]` — Admin: create coupon\n"
        "`!coupon delete <id>` — Admin: delete coupon"
    ), inline=False)
    billing_embed.add_field(name="🖥️ Plans & Deploy", value=(
        "`!plans` — Show all monthly plans\n"
        "`!deploy <plan>` — Deploy a plan\n"
        "Each plan can be deployed **2 times per user**."
    ), inline=False)
    pages["billing"] = billing_embed

    port_embed = create_embed("📚 PrimeCloud Help - 🔌 Port Forwarding", "Manage ports for your VPS.", 0x00ccff)
    port_embed.add_field(name="⌯⌲ Commands", value=(
        "`!ports` — Port forwarding help\n"
        "`!pcreate <vps_num> <port>` — Add port forward\n"
        "`!ports list` — List port forwards\n"
        "`!ports remove <id>` — Remove port forward\n"
        "Port `22` is reserved for SSH/Tailscale."
    ), inline=False)
    pages["ports"] = port_embed

    tools_embed = create_embed("🔧 VPS Tools", "Extra tools to manage your VPS:", 0x00ccff)
    tools_embed.add_field(name="📊 Monitoring", value=(
        "`!ping-vps <#>` — Ping VPS\n"
        "`!uptime-vps <#>` — VPS uptime\n"
        "`!myinfo` — Profile, balance and VPS summary"
    ), inline=False)
    tools_embed.add_field(name="🏷️ Customization", value=(
        "`!rename-vps <#> <name>` — Set a VPS nickname\n"
        "`!vps-note <#> <text>` — Add/update a VPS note"
    ), inline=False)
    pages["tools"] = tools_embed

    extras_embed = create_embed("📢 Extras", "PrimeCloud utilities:", 0xff6b9d)
    extras_embed.add_field(name="ℹ️ Commands", value=(
    ...
    if is_user_admin:
        ...
        admin_embed.add_field(name="🎟️ Billing & Coupon", value=(
            "`!coupon create <code> <expiry> <slots> [amount]` — Create a coupon\n"
            "`!coupon delete <id>` — Delete a coupon\n"
            "`!adminc @user <amount>` — Add legacy credits\n"
            "`!adminrc @user <amount/all>` — Remove legacy credits"
        ), inline=False)
        admin_embed.add_field(name="🖥️ VPS Control", value=(
            "`!create @user <ram> <cpu> <disk>` — Create VPS\n"
            "`!delete-vps @user <#> <reason>` — Delete VPS\n"
            "`!restart-vps <container>` — Restart VPS\n"
            "`!stop-vps-all` — Stop all VPS\n"
            "`!exec <container> <cmd>` — Execute command"
        ), inline=False)
        admin_embed.add_field(name="📊 Admin Tools", value=(
            "`!userinfo @user` — User information\n"
            "`!serverstats` — Server stats\n"
            "`!vpsinfo [container]` — VPS information\n"
            "`!announce <message>` — DM VPS owners\n"
            "`!maintenance <on/off>` — Maintenance mode"
        ), inline=False)
        pages["admin"] = admin_embed

    if is_user_main_admin:
        mainadmin_embed = create_embed("👑 Main Admin", "Main admin commands:", 0xffd700)
...
async def show_help(ctx):
    ...
    options = [
        ...
        discord.SelectOption(label="💳 Billing & Coupon", description="Balance, coupons & VPS plans", value="billing", emoji="💳"),
...
async def on_message(message):
    ...
    # ─── EARNING TASK TRACKER ─────────────────────────────────────────────────
    # Counts only exact task-code messages, across server text channels.
    if message.guild is not None:
        uid = str(message.author.id)
        task = billing_data.setdefault("earning_tasks", {}).get(uid)
        if task and not task.get("completed") and message.content.strip() == task.get("code"):
            task["count"] = int(task.get("count", 0)) + 1
            target = int(task.get("target", TASK_MIN_MESSAGES))
            if task["count"] >= target:
                reward = random.randint(TASK_MIN_REWARD, TASK_MAX_REWARD)
                set_balance(uid, get_balance(uid) + reward)
                task["completed"] = True
                task["completed_at"] = datetime.utcnow().timestamp()
                task["reward"] = reward
                save_data()
                await message.channel.send(embed=create_success_embed(
                    "🎉 Task Completed!",
                    f"{message.author.mention} completed the task with **{target} messages**.\n"
                    f"Lucky reward: **₹{reward:,}**\n"
                    f"New balance: **₹{get_balance(uid):,.2f}**"
                ))
            else:
                # Save occasionally so a restart doesn't lose much progress.
                if task["count"] % 10 == 0:
                    save_data()

    # ─── PRIMECLOUD_AI_PING_HANDLER ────────────────────────────────────────────
    #
    ...
    if (
        ...
        try:
            ...
            if question:
                ...
                # Built-in PrimeCloud identity responses
                direct = primecloud_ai_direct_answer(question)
                ...
                if direct:
                    ...
                    if not OPENROUTER_API_KEY:
                        answer = (
                            "🤖 PrimeCloud AI is currently not configured.\n"
                            "Please contact the PrimeCloud administrator."
                        )
                    else:
                        async with message.channel.typing():
                            answer = await primecloud_ai_answer(question)
            ...
            logger.error(
                f"PrimeCloud AI error: {ai_error}"
            )
...
async def _primecloud_sync_commands():
    try:
        synced = await bot.tree.sync()
        print(f"[PrimeCloud] Synced {len(synced)} slash commands", flush=True)
    except Exception as e:
        print(f"[PrimeCloud] Slash sync error: {e}", flush=True)

bot.add_listener(_primecloud_sync_commands, 'on_ready')
