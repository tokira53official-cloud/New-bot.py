...
If there is no requirements.txt file, install the Python packages required by bot.py manually.
...
If your bot.py already reads the token from an environment variable, use that method instead of putting the token directly inside the source code.
...
IMPORTANT:
Never put the real auth key inside bot.py, README.txt, GitHub commits, screenshots, or any other public file.
